...
const App = () => {
  ...
  // Replace this value to filter based on the selected room
  const selectedRoomId = 5;
   ...
   * Includes user information and filters by room ID.
   */
   ...
   * Filters by `selectedRoomId`.
   */
  const parseBookings = (bookings) => {
    return bookings
      .filter((booking) => booking.room_id === selectedRoomId) // Filter by room
      .map((booking) => ({
        id: booking.id,
