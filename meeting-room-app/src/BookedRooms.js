...
function BookedRooms() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      // Extract first 5 characters of admin's company_id
      const adminCompanyIdPrefix = userData.company_id.slice(0, 5);

      fetchBookings(
        userData.role,
        userData.email,
        userData.id,
        adminCompanyIdPrefix
      );
    }
  }, []);
  ...
  const fetchBookings = async (role, email, userId, adminCompanyIdPrefix) => {
    try {
      const response = await axiosInstance.get("/bookings", {
        params: { role, email, user_id: userId },
      });

      // Filter bookings to show only those that match the admin's company_id prefix
      const filteredBookings = response.data.filter((booking) => {
        if (!booking.user || !booking.user.company_id) return false;

        const userCompanyIdPrefix = booking.user.company_id.slice(0, 5);
        return userCompanyIdPrefix === adminCompanyIdPrefix;
      });
  ...
  const handleDeleteBooking = async (bookingId) => {
    try {
      ...
      // Ensure the admin's company ID prefix is used when refetching bookings
      const adminCompanyIdPrefix = user.company_id.slice(0, 5);
      fetchBookings(user.role, user.email, user.id, adminCompanyIdPrefix);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <table border="1" className="table text-center">
        <thead className="table-dark card-header">
          <tr>
            <th>Person Name</th>
            <th>Person Email</th>
            <th>Date</th>
